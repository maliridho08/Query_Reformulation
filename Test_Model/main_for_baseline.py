...
def main():
    ...
    for x,query in enumerate(daftar_query):
        ...
        for indeks,word in enumerate(query.split()):
            ...
            word_search_result[indeks][0]=word                                  #eg: input apa in index [0][0] then ibukota in index [1][0] then indonesia in index [2][0]
            if word in model.wv.key_to_index:   #check if word is in model 
                hasil = model.wv.most_similar(word, topn=total_similar_word)    #eg: will search similair word of apa, then ibukota then indonesia
                ...
                word_search_result[indeks][0] = word+"_x"   #if word not in model, add _x ; for readable result
